...
def plotGTC(chains, **kwargs):
    ...
    try:  # Not a list of DFs, but might be a single DF
        ...
        if hasattr(chains[0], 'columns'):
            # Set param names from DataFrame column names, can be overridden
            # later
    ...
    assert nChains <= len(defaultColorsOrder), \
        ...
        str(len(defaultColorsOrder)) + " chains"
    ...
    if chainLabels is not None:
        ...
        assert len(
            chainLabels) == nChains, "chainLabels mismatch with number of chains"
        # Check that it's a list of strings
        assert all(__isstr(s)
                   for s in chainLabels), "chainLabels must be list of strings"
    ...
    if paramNames is not None:
        ...
        assert len(
            paramNames) == nDim, "paramNames mismatch with number of dimensions"
        # Check that it's a list of strings
        assert all(__isstr(s)
                   for s in paramNames), "paramNames must be list of strings"
    ...
    if paramRanges is not None:
        assert len(
            paramRanges) == nDim, "paramRanges must match number of parameters"
    ...
    # Default supports up to three truths
    ...
    if truthLabels is not None:
        ...
        # Check that it's a list of strings
        assert all(__isstr(s)
                   for s in truthLabels), "truthLabels must be list of strings"
        assert len(truthLabels) == len(
            truths), "truthLabels mismatch with number of truths"
    ...
    if priors is not None:
        if haveScipy:
            assert len(
                priors) == nDim, "List of priors must match number of parameters"
            for i in range(nDim):
                if priors[i]:
                    assert priors[i][1] > 0, "Prior width must be positive"
        else:
            warnings.warn(
                "You need to have scipy installed to display Gaussian priors, ignoring priors keyword.", UserWarning)
            priors = None
    ...
    if weights is None:
        ...
        for i in range(nChains):
            assert len(weights[i]) == len(chains[i]), \
                "missmatch in chain/weights #%d: len(chain) %d, len(weights) %d" % (
    ...
    # Maintain support for older naming convention. TODO: Remove in next major
    # version
    ...
    if deprecated_ConfLevels:
        warnings.warn(
            "gaussianConfLevels has been replaced by sigmaContourLevels", DeprecationWarning)
        confLevels = (.6065, .1353, .0111)
    deprecated_ConfLevels = kwargs.pop('GaussianConfLevels', False)
    if deprecated_ConfLevels:
        warnings.warn(
    ...
    # Number of bins for 1d and 2d histograms. 30 works...
    nBins = kwargs.pop('nBins', 30)
    # Don't you like smooth data?
    smoothingKernel = kwargs.pop('smoothingKernel', 1)
    if (smoothingKernel != 0) and (not haveScipy):
        warnings.warn(
            "Warning: You don't have Scipy installed. Your curves will not be smoothed.", UserWarning)
        smoothingKernel = 0
    ...
    # provide your own
    # Figure size descriptor or figure width=height in inches
    figureSize = kwargs.pop('figureSize', None)
    if figureSize is None:
        # If no figure size is given, use resolution of 70 ppp (pixel per
        # panel)
    ...
    # Set font in rcParams (Not in the default file, but just in the running
    # kernel)
    ...
    if 'ylabelpad' in customLabelFont.keys():
        ...
        ylabel_kwargs['labelpad'] = customLabelFont['ylabelpad']
            ...
            for j in range(nDim):  # column
                if j < i:
                    ...
                    if xTicks[j] is None:
                        ...
                        # Remove xticks that are too close (5% of panel size)
                        # to panel edge
    ...
    if do1dPlots:
        ...
        for i in range(nDim):
            ...
            if i == nDim - 1:
                # 5 ticks max
                ax.xaxis.set_major_locator(mtik.MaxNLocator(5))
                # Remove xticks that are too close (5% of panel size) to panel
                # edge
    ...
    if len(axH) > 1:
        ...
        for i in range(len(axH)):
            bboxTickLabel = axH[i].xaxis.get_ticklabel_extents(renderer)[
                0].get_points()
            bboxSize[i] = bboxTickLabel[1, 1] - bboxTickLabel[0, 1]
            panelWidth = axH[i].get_window_extent().transformed(
        ...
        for i in range(len(axV)):
            bboxTickLabel = axV[i].yaxis.get_ticklabel_extents(renderer)[
                0].get_points()
            bboxSize[i] = bboxTickLabel[1, 0] - bboxTickLabel[0, 0]
            panelHeight = axV[i].get_window_extent().transformed(
...
def __plot1d(ax, nChains, chains1d, weights, nBins, smoothingKernel,
    ...
    if prior1d is not None:
        ...
        arr = np.linspace(prior1d[0] - 4 * prior1d[1],
                          prior1d[0] + 4 * prior1d[1], 40)
        ax.plot(arr, norm.pdf(arr, prior1d[0], prior1d[
                1]), lw=1, color=priorColor)
    ...
    for k in reversed(range(nChains)):
        ...
        if (np.isnan(chains2d[k][0]).all()) | (np.isnan(chains2d[k][1]).all()):
            ...
            for l in range(nContourLevels):
                ...
                chainLevels[k][nContourLevels - 1 -
                               l] = np.interp(temp, nBinsFlat, histOrdered)

            # Apply Gaussian smoothing and plot filled contours if requested
            if smoothingKernel > 0:
                plotData.append(scipy.ndimage.gaussian_filter(
    ...
    if truths2d is not None:
        for k in range(len(truths2d)):
            ...
            if truths2d[k][0] is not None:
                ax.axhline(truths2d[k][0], lw=1, color=truthColors[
                           k], ls=truthLineStyles[k])
            # vertical line
            if truths2d[k][1] is not None:
                ax.axvline(truths2d[k][1], lw=1, color=truthColors[
                           k], ls=truthLineStyles[k])
